...
ncores <- strtoi(Sys.getenv("SLURM_CPUS_PER_TASK"))
sprintf("Number of cores: %d", ncores)
set_cmdstan_path(path = "/home/verrett/R/x86_64-pc-linux-gnu-library/4.2/cmdstanr/R")

#brm 
#might want to look into if parasite random effects should be added
#for now just try pgls version with averaged residuals
#see example 2 for chkpt brms; can use test partition to try
bmod=brm(average_jackknife~urban+Migratory_status+lcites+
        (1|gr(label,cov=A)),
...
save.image(file='urbmigmod1_noLeuco_f9.RData')
