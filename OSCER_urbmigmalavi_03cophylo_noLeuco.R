...
#setwd("~/Desktop/urbmigmalavi/flat files")
setwd("/Users/taylorverrett/Documents/GitHub/urbmigmalavi/flat files")
saveRDS(pac,"pac_noLeuco.rds")
saveRDS(pac_links,"pac_links_noLeuco.rds")
write.csv(rdata,"PACo results_noLeuco.csv")
...
use.edge.length=F,lwd=0.05,space=500,gap=1,length.line=-5)

cophyloplot(htree,ptree,assoc=imat,show.tip.label=F,
            use.edge.length=F,lwd=wei,space=120,gap=1,length.line=0)
